...
class PemanfaatanMroController extends TrinataController
{
    ...
    public function getData()
    {
        ...
        $model = $this->model->select('id','warehouse_id','name','komag','category', 'year_acquisition','amount','total_proposed_amount','unit_price','unit')->whereType('mro')->orderBy('created_at','desc');
    ...
    public function postAjukan(Request $request)
    {
        ...
        // dd($inputs,$cart,\Auth::user()->head()->first());

        if ($head->warehouse_id==0) {
            return redirect(urlBackend('pemanfaatan-mro/ajukan'))->withInfo('data warehouse empty');
        }else{
