...
class PemanfaatanMroAbtController extends TrinataController
{
    ...
    public function getData()
    {
        ...
        $model = $this->model->select('id','warehouse_id','name','komag','category', 'year_acquisition','amount','unit_price','unit')->whereType('mroabt')->orderBy('created_at','desc');
    ...
    public function postAjukan(Request $request)
    {
        ...
        if ($head->warehouse_id==0) {
            return redirect(urlBackend('pemanfaatan-mro-abt/ajukan'))->withInfo('data warehouse empty');
        }else{
