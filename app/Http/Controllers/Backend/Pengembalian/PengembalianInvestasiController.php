...
class PengembalianInvestasiController extends TrinataController
{
   ...
   public function __construct(Reversion $reversion, ReversionDetail $reversiondetail, Utilization $model, UtilizationDetail $detail)
    {
        ...
        $this->resource = "backend.pengembalian.investasi.";
    }

    public function getData()
    {
        $model = $this->model->select()->where('type','investasi')->where('status',5);
    ...
    public function postAjukan(Request $request)
    {
        ...
        if ($head->warehouse_id==0) {
            return redirect(urlBackend('pengembalian-investasi/ajukan'))->withInfo('data warehouse empty');        
        }else{
