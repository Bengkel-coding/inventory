            ...
            <div class="col-md-12 fadeIn animated">        
              <ul class="nav nav-tabs">
                ...
                <li class="@if ($request->type == 'pending') active @endif">
                  <a href="{{urlBackend('dashboard/mutation?type=pending')}}">
                        Mutasi Pending
                  </a>
                </li>
                <li class="@if ($request->type == 'approved') active @endif">
                  <a href="{{urlBackend('dashboard/mutation?type=approved')}}">
                        Mutasi Approved
                  </a>
                </li>
                <li class="@if ($request->type == 'process') active @endif">
                  <a href="{{urlBackend('dashboard/mutation?type=process')}}">
                        Mutasi Process
                  </a>
                </li>
              </ul>
        ...
        $(document).ready(function(){
          ...
          // var table =  $('#table').DataTable({
            $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ urlBackendAction("data-mutation?type=$request->type") }}',
                columns: [
                    { data: 'type', name: 'type' },
                    { data: 'name', name: 'name'},
